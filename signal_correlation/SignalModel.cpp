...
namespace DSP {
    ...
    double SineSignalASK::sampleBit(int bit, UnitDSP::Seconds timePoint) {
        ...
        phase = 2 * pi * getCarrierFrequency() * timePoint;
        return amplitude;
    }
    ...
    double SineSignalBPSK::sampleBit(int bit, UnitDSP::Seconds timePoint) {
        double amplitude{ sin(phase + bit * pi) };
        phase = 2 * pi * getCarrierFrequency() * timePoint;
        return amplitude;
    }
    ...
    double SineSignalMSK::sampleBit(int bit, UnitDSP::Seconds timePoint) {
        ...
        if (bit == 0) {
            phase += 2 * pi * getCarrierFrequency() * dt;
        }
        else {
            phase += 2 * pi * (getCarrierFrequency() + frequencyDiff_) * dt;
        }
        return amplitude;
    }
